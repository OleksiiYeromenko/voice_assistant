...
class TemperatureLabel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        header.setStyleSheet(f"color: {theme.TEXT_SECONDARY}; font-size: 12px;")

        self._value = QLabel("—", self)
        self._value.setStyleSheet(f"color: {theme.COLOR_OK}; font-size: 14px; font-weight: bold;")
    ...
    def update_temp(self, celsius: float | None):
        if celsius is None:
            self._value.setText("—")
            self._value.setStyleSheet(f"color: {theme.TEXT_MUTED}; font-size: 14px; font-weight: bold;")
        else:
            color = theme.temp_color(celsius)
            self._value.setText(f"{celsius:.0f}°C")
            self._value.setStyleSheet(f"color: {color}; font-size: 14px; font-weight: bold;")
...
class LatencyPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        style = f"color: {theme.TEXT_SECONDARY}; font-family: 'DejaVu Sans Mono'; font-size: 12px;"
        ...
        self._model.setStyleSheet(
            f"color: {theme.TEXT_MUTED}; font-family: 'DejaVu Sans Mono'; font-size: 11px;"
        )
...
class SysBar(QWidget):
    """Bottom bar (72px): CPU + RAM gauges, temperature, latency panel."""
